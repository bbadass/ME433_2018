...
public class MainActivity extends Activity implements TextureView.SurfaceTextureListener {
    ...
    int threshred = 0; // comparison value for red
    ...
    public void onSurfaceTextureUpdated(SurfaceTexture surface) {
        ...
        if (c != null) {
            for (int k=0; k < bmp.getHeight(); k++) {//loop over rows of pixels
                ...
                for (int i = 0; i < bmp.getWidth(); i++) {//loop over every element of pixels
                    if ((green(pixels[i]) - red(pixels[i])) > threshred && (green(pixels[i]) - 0.7*blue(pixels[i])) > threshblue) { //adjust based on how green is the definition of green
